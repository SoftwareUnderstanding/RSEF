...
from ..metadata.api.openAlex_api_queries import query_openalex_by_title
...
def meta_to_dwnldd(metadataObj, output_dir, pdf_link=None):
    ...
    try:
        ...
        file_path = pdf_download_pipeline(
            id=doi, output_directory=output_dir, pdf_link=pdf_link)
        if not file_path:
            return None
        file_name = os.path.basename(file_path)

        if not metadataObj or metadataObj and not metadataObj.title:
            title = extract_pdf_title(file_path)

            if title:
                # Intend to use OpenAlex API to get metadata
                oa_meta = query_openalex_by_title(title)

                metadataObj = metadataObj or MetadataObj(
                    title=None, doi=None, arxiv=None, publication_date=None, authors=None)

                metadataObj.title = title

                if oa_meta:
                    full_title = safe_dic(oa_meta, "title")
                    if full_title:
                        metadataObj.title = full_title
                    ...
                    for author in authorships:
                        authors.append(
                            safe_dic(safe_dic(author, "author"), "display_name"))

                    metadataObj.doi = doi
                    metadataObj.arxiv = arxiv
                    metadataObj.publication_date = publication_date
        ...
        try:
            ...
            log.error(
                "Error while creating the downloaded object with this doi: %s for due to %s", meta_doi, str(e))
        except Exception as e:
            log.error("Error while creating the downloaded object with this metadataObj: %s for due to %s", str(
                metadataObj), str(e))
        return None
...
def downloadedDic_to_downloadedObj(dwnldd_dict):
    ...
    pdf_link = safe_dic(dwnldd_dict, "pdf_link")
    return DownloadedObj(title=title, doi=doi, arxiv=arxiv, publication_date=publication_date, authors=authors, file_name=file_name, file_path=file_path, pdf_link=pdf_link)
...
def metaJson_to_downloaded_dic(meta_json, output_dir):
    ...
    for doi in metas_dict:
        ...
        dwnObj = metaDict_to_downloaded(
            meta_dict=meta_dict, output_dir=output_dir)
...
def doi_to_downloadedObj(doi, output_dir):
    ...
    if meta or pdf_link:
        return meta_to_dwnldd(meta, output_dir, pdf_link=pdf_link)
    else:
...
def _doi_to_downloaded_obj_backup(id, output_dir):
    try:
        ...
        log.error(
            f"An error occurred in _doi_to_downloaded_obj_backup: {str(e)}")
        return None
...
def pdf_to_downloaded_obj(pdf, output_dir):
    ...
    if not (title := extract_pdf_title(pdf_path=pdf)):
        return None
    resp_jsn = query_openalex_by_title(title)
...
def download_from_pdf_url(url, output_dir):
    ...
    if os.path.exists(output_path):
        log.info(
            f"File {filename} already exists in {output_dir}. Skipping download.")
        return output_path
    ...
    if response.status_code == 200:
        ...
        if content_type == 'application/pdf':
            ...
            log.debug(
                f"The content at {url} is not a PDF. Content-Type is {content_type}")
            return None
    else:
        log.debug(
            f"Error downloading PDF from {url}: Status code {response.status_code}")
        return None
        ...
        if primary_location is not None:
            ...
            if downloaded:
                if doi is not None and doi.startswith("10."):
                    ...
                    downloadedObj = DownloadedObj(title=name, doi=doi, arxiv=downloadedMeta.arxiv, file_name=os.path.basename(
                        primary_location), file_path=downloaded).to_dict()
                else:
                    downloadedObj = DownloadedObj(title=name, doi="", arxiv="", file_name=os.path.basename(
                        primary_location), file_path=downloaded).to_dict()
                if downloadedObj:
